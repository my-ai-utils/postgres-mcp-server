...
pub struct AppContext {
    ...
    /// Expiry (`unix_microseconds`) of the current write-access window; `0` means
    /// disabled. Stored as the deadline rather than a flag plus a timer, so it
    /// expires lazily on read and needs no background task.
    ///
...
impl AppContext {
    ...
    /// Adds [`MCP_WRITES_WINDOW`] to the write window, opening it if it was
    /// closed. Each call stacks, so three calls grant 30 minutes.
