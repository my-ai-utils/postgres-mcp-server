...
The list is **grouped by `applicationName`** — that is the question a connection count answers into, *which service* is holding them — with the clients that set no name last, and inside each application the busiest first: active, then `idle in transaction`, then idle, each longest-in-state first.

That second ordering is also the one the **100-row cap** is applied by, before the grouping, and the two are deliberately not the same sort: cutting an alphabetical list at a hundred drops every application after some letter, which is the one way of choosing a hundred rows that says nothing. Cut this way, what a truncated list loses is its dullest end — and the number shown is always published against `inThisDb`, the total it was cut from. It is also the only place an `idle in transaction` backend turns up: it is in no slow-query list, because it is not running anything — while it goes on holding every lock it took and pinning the xmin horizon against `VACUUM`. The UI marks that column once a backend has sat there for 5 seconds and reddens it at a minute.
